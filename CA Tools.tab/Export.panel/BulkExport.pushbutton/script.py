...
# ===========================================================================
#  EXPORT PROFILES  (whole-window settings presets, persisted per user)
# ===========================================================================
def _profiles_path():
    return op.join(_ca_config_dir(), "bulk_export_profiles.json")


def load_profiles():
    """Return {'last': name, 'profiles': [ {name, settings}, ... ]} (never raises)."""
    try:
        p = _profiles_path()
        if not op.isfile(p):
            return {"last": None, "profiles": []}
        with open(p, "r") as fh:
            data = json.load(fh)
        if not isinstance(data, dict):
            return {"last": None, "profiles": []}
        prof = []
        for f in data.get("profiles", []) or []:
            try:
                nm = f.get("name")
                if nm:
                    prof.append({"name": nm,
                ...
                pass
        return {"last": data.get("last"), "profiles": prof}
    except Exception:
        return {"last": None, "profiles": []}


def save_profiles(data):
    """Persist the profiles store. Returns True on success."""
    try:
...
def read_settings_file(path):
    """Read a single settings dict from a JSON *or* XML file (never raises)."""
    if not path:
        return None
    low = path.lower()
    if low.endswith(".xml"):
        res = read_profile_xml(path)
        return res[1] if res else None
    # Try JSON first, fall back to XML by content.
    try:
...
def _xml_parse_bool(s):
    return str(s).strip().lower() in ("true", "1", "yes")


def settings_to_xml(name, settings):
    """Serialise a profile (name + settings) to an XML string."""
    import xml.etree.ElementTree as ET
    root = ET.Element("CAToolsBulkExportProfile")
    root.set("name", name or "")
    root.set("version", "1")

    def _dictblock(tag, d, extra=None):
        el = ET.SubElement(root, tag)
        for k in sorted((d or {}).keys()):
            it = ET.SubElement(el, "item")
            it.set("key", str(k))
            v = d[k]
            if extra == "bool":
                it.set("value", _xml_bool(v))
            elif extra == "combo":
                it.set("index", str(v.get("index", -1)))
                txt = v.get("text")
                if txt is not None:
                    it.set("text", txt)
            else:
                it.set("value", "" if v is None else str(v))
        return el

    _dictblock("checks", settings.get("checks"), "bool")
    _dictblock("radios", settings.get("radios"), "bool")
    _dictblock("texts", settings.get("texts"))
    _dictblock("combos", settings.get("combos"), "combo")

    naming = ET.SubElement(root, "naming")
    fs = settings.get("field_sep")
    if fs is not None:
        naming.set("fieldSep", fs)
    naming.set("printOrderMode", settings.get("print_order_mode") or "number")
    for t in settings.get("tokens") or []:
        te = ET.SubElement(naming, "token")
        te.set("kind", str(t.get("kind", "")))
        te.set("value", str(t.get("value", "")))
...
def write_profile_xml(path, name, settings):
    """Write a profile to an XML file. Returns True on success."""
    try:
        xml_text = settings_to_xml(name, settings)
...
def read_profile_xml(path):
    """Read (name, settings) from a profile XML file, or None on failure."""
    import xml.etree.ElementTree as ET
    try:
        tree = ET.parse(path)
        root = tree.getroot()
    ...
    try:
        ...
        settings = {"checks": {}, "radios": {}, "texts": {}, "combos": {},
                    "tokens": [], "field_sep": None,
                    "print_order_mode": "number"}
        for it in root.findall("./checks/item"):
            settings["checks"][it.get("key")] = _xml_parse_bool(it.get("value"))
        for it in root.findall("./radios/item"):
            settings["radios"][it.get("key")] = _xml_parse_bool(it.get("value"))
        for it in root.findall("./texts/item"):
            settings["texts"][it.get("key")] = it.get("value", "")
        for it in root.findall("./combos/item"):
            try:
                idx = int(it.get("index", "-1"))
            except Exception:
                idx = -1
            settings["combos"][it.get("key")] = {
                "index": idx, "text": it.get("text")}
        naming = root.find("./naming")
        if naming is not None:
            settings["field_sep"] = naming.get("fieldSep", None)
            settings["print_order_mode"] = naming.get("printOrderMode", "number")
            for te in naming.findall("./token"):
                settings["tokens"].append(
                    {"kind": te.get("kind", ""), "value": te.get("value", "")})
        return (name, settings)
    except Exception:
        return None


# Controls whose state makes up a profile. Each access is guarded, so a missing
# or unexpected control is skipped rather than raising.
_PROFILE_CHECKS = [
    "chk_reverse", "chk_only_set", "chk_pdf", "chk_dwg",
    "chk_halftone", "chk_hidecrop", "chk_hideref", "chk_hidescope",
    "chk_hidetags", "chk_keepjump", "chk_maskcoin", "chk_viewlinks",
]
_PROFILE_RADIOS = [
    "rb_sheets", "rb_views", "rb_combine", "rb_separate",
    "rb_save_same", "rb_save_split", "rb_vector", "rb_raster",
    "rb_center", "rb_offset", "rb_fit", "rb_zoom",
]
_PROFILE_TEXTS = [
    "txt_folder", "txt_filename", "txt_index_start",
    "txt_offx", "txt_offy", "txt_zoom",
]
_PROFILE_COMBOS = [
    "cmb_view_filter", "cmb_index_digits", "cmb_colors", "cmb_margin",
    "cmb_paper_orient", "cmb_raster", "cmb_dwg_setup", "cmb_report",
]
...
#  NEW-PROFILE DIALOG
# ===========================================================================
class ProfileCreateDialog(forms.WPFWindow):

    def __init__(self):
        forms.WPFWindow.__init__(self, _bundle("ProfileDialog.xaml"))
        self.ok = False
        self.profile_name = ""
        self.mode = "copy"                 # copy | default | import
        self.imported_settings = None

    def on_create(self, sender, args):
        name = (self.txt_name.Text or "").strip()
        if not name:
            forms.alert("Enter a profile name.", title="New Profile")
            return
        if self.rb_import.IsChecked:
            ...
            try:
                ...
                ofd = OpenFileDialog()
                ofd.Title = "Import profile from file"
                ofd.Filter = ("Profile files (*.xml;*.json)|*.xml;*.json|"
                              "XML profile (*.xml)|*.xml|"
            ...
            if settings is None:
                forms.alert("That file could not be read as profile settings.",
                            title="New Profile")
                return
            self.mode = "import"
            self.imported_settings = settings
        elif self.rb_default.IsChecked:
            self.mode = "default"
        else:
            self.mode = "copy"
...
class BulkExportWindow(forms.WPFWindow):
    ...
    def __init__(self):
        ...
        self._prof_store = {"last": None, "profiles": []}
        # Snapshot the freshly-initialised UI as the "default settings" so a
        # profile created with "Use default settings" is reproducible.
        try:
    ...
    def _collect_settings(self):
        """Snapshot every profile-relevant control into a plain dict."""
        s = {"checks": {}, "radios": {}, "texts": {}, "combos": {},
             "tokens": list(self._tokens), "field_sep": self._field_sep,
             "print_order_mode": self._print_order_mode}
        for n in _PROFILE_CHECKS:
            try:
                s["checks"][n] = bool(getattr(self, n).IsChecked)
            except Exception:
                pass
        for n in _PROFILE_RADIOS:
            try:
                s["radios"][n] = bool(getattr(self, n).IsChecked)
            except Exception:
                pass
        for n in _PROFILE_TEXTS:
            try:
                s["texts"][n] = str(getattr(self, n).Text)
            except Exception:
                pass
        for n in _PROFILE_COMBOS:
            try:
                c = getattr(self, n)
                s["combos"][n] = {
                    "index": int(c.SelectedIndex),
                    "text": (str(c.SelectedItem)
                             if c.SelectedItem is not None else None)}
            except Exception:
                pass
        return s

    def _apply_settings(self, s):
        """Push a settings dict back onto the UI (guarded per control)."""
        if not s:
            return
        if "tokens" in s:
            self._tokens = list(s.get("tokens") or [])
        if "field_sep" in s:
            self._field_sep = s.get("field_sep")
        if "print_order_mode" in s:
            self._print_order_mode = s.get("print_order_mode") or "number"

        for n, v in (s.get("texts") or {}).items():
            try:
                getattr(self, n).Text = v
            except Exception:
                pass
        for n, v in (s.get("combos") or {}).items():
            try:
                c = getattr(self, n)
                txt = v.get("text")
                idx = v.get("index", -1)
                applied = False
                if txt is not None:
                    for i in range(c.Items.Count):
                        if str(c.Items[i]) == txt:
                            c.SelectedIndex = i
                            applied = True
                            break
                if not applied and isinstance(idx, int) and 0 <= idx < c.Items.Count:
                    c.SelectedIndex = idx
            except Exception:
                pass
        for n, v in (s.get("checks") or {}).items():
            try:
                getattr(self, n).IsChecked = bool(v)
            except Exception:
                pass
        for n, v in (s.get("radios") or {}).items():
            try:
                getattr(self, n).IsChecked = bool(v)
            except Exception:
                pass

        # Reflect the applied settings everywhere that matters.
        try:
            self.on_format_toggle(None, None)
        except Exception:
            pass
    ...
    def _init_profiles(self):
        self._prof_store = load_profiles()
        self._loading_profiles = True
        try:
            self.cmb_profile.Items.Clear()
            for p in self._prof_store["profiles"]:
                self.cmb_profile.Items.Add(p["name"])
        except Exception:
            pass
        self._loading_profiles = False
        # Auto-select and apply the last-used profile, if any.
        last = self._prof_store.get("last")
        if last:
            for i, p in enumerate(self._prof_store["profiles"]):
                if p["name"] == last:
                    self._loading_profiles = True
                    try:
                        self.cmb_profile.SelectedIndex = i
                    except Exception:
                        pass
                    self._loading_profiles = False
                    self._apply_settings(p.get("settings"))
                    break
    ...
    def _selected_profile(self):
        try:
            i = self.cmb_profile.SelectedIndex
            if 0 <= i < len(self._prof_store["profiles"]):
                return self._prof_store["profiles"][i]
        except Exception:
            pass
        return None
    ...
    def on_profile_changed(self, sender, args):
        ...
        self._apply_settings(p.get("settings"))
        try:
            self._prof_store["last"] = p["name"]
            save_profiles(self._prof_store)
        except Exception:
            pass

    def on_profile_add(self, sender, args):
        if not getattr(self, "_ready", False):
            return
        dlg = ProfileCreateDialog()
        try:
            dlg.Owner = self
        except Exception:
            pass
        ...
        elif dlg.mode == "import":
            settings = dlg.imported_settings or {}
        else:
        ...
        for i, p in enumerate(store["profiles"]):
            if p["name"] == name:
                if not forms.alert(
                        ...
                        "Overwrite it?".format(name),
                        title="New Profile", yes=True, no=True):
                    return
                store["profiles"][i] = {"name": name, "settings": settings}
                replaced = True
                break
        if not replaced:
            store["profiles"].append({"name": name, "settings": settings})
        store["last"] = name
        if save_profiles(store):
            self._refresh_profile_combo(select_name=name)
            self._apply_settings(settings)
            # Also let the user keep a portable copy of the profile on their PC.
            # The profile is already saved internally; this XML is for reuse /
            # sharing. Cancelling the dialog is fine and leaves the profile in
            # place.
            self._prompt_save_profile_file(
                name, settings,
                title="Save new profile '{0}' as XML".format(name))
        else:
            forms.alert("Could not save the profile to disk.",
                        title="New Profile")
    ...
    def on_profile_save(self, sender, args):
        ...
        p = self._selected_profile()
        if not p:
            forms.alert("Select a profile first, or use + to create one.",
                        title="Save Profile")
            return
        store = load_profiles()
        found = False
        for q in store["profiles"]:
            if q["name"] == p["name"]:
                q["settings"] = self._collect_settings()
                found = True
                break
        if not found:
            store["profiles"].append(
                {"name": p["name"], "settings": self._collect_settings()})
        store["last"] = p["name"]
        if save_profiles(store):
            self._prof_store = store
        else:
            forms.alert("Could not save the profile to disk.",
                        title="Save Profile")
    ...
    def on_profile_delete(self, sender, args):
        ...
        if not p:
            ...
            return
        if not forms.alert("Delete the profile '{0}'?".format(p["name"]),
                           title="Delete Profile", yes=True, no=True):
            return
        store = load_profiles()
        store["profiles"] = [q for q in store["profiles"]
                             if q["name"] != p["name"]]
        if store.get("last") == p["name"]:
            store["last"] = None
        if save_profiles(store):
            self._refresh_profile_combo()
        else:
            forms.alert("Could not update the profiles on disk.",
                        title="Delete Profile")

    def _prompt_save_profile_file(self, name, settings, title="Save profile to file"):
        """Pop up a Save dialog and write the profile to XML (or JSON).
        ...
        try:
            ...
            sfd = SaveFileDialog()
            sfd.Title = title
            sfd.Filter = ("XML profile (*.xml)|*.xml|"
                          "JSON profile (*.json)|*.json")
            sfd.FileName = "{0}.xml".format(sanitize(name) or "profile")
        ...
        if path.lower().endswith(".json"):
            try:
                with open(path, "w") as fh:
                    json.dump({"name": name, "settings": settings}, fh, indent=2)
                ok = True
            except Exception:
                ok = False
        else:
            ok = write_profile_xml(path, name, settings)
        if not ok:
            forms.alert("Could not save the profile to that file.",
                        title="Save Profile")
            return None
        return path

    def on_profile_export(self, sender, args):
        """Save the selected profile (or current settings) to an XML/JSON file."""
        if not getattr(self, "_ready", False):
            return
        p = self._selected_profile()
        if p:
            ...
            name = "profile"
            settings = self._collect_settings()
        self._prompt_save_profile_file(name, settings)
